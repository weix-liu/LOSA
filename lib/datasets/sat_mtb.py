...
class sat_mtb(imdb):
    def __init__(self, image_set, year, devkit_path=None):
        imdb.__init__(self, 'sat_mtb' + '_' + image_set)
        self._year = year
        self._image_set = image_set
        self._devkit_path = cfg_d.SAT_MTB
    ...
    def _load_pascal_annotation(self, index):
        ...
        objs1 = tree.findall('object')
        select_objs = []
        for ix, obj in enumerate(objs1):
            cls_name = obj.find('name').text.lower().strip()
            if cls_name  in self._class_to_ind.keys():
